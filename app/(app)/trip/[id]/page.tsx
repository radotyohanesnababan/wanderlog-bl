...
export default async function TripDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: places } = await supabase
    .from("places").select("*, reviews(*)")
    .eq("trip_id", trip.id);

  const { data: tripPoints } = await supabase
    .from("trip_points").select("lat, lng")
    .eq("trip_id", trip.id)
  ...
  return (
    <main className="min-h-screen p-4 max-w-lg mx-auto pb-8">
      ...
      <div className="bg-slate-800 border border-slate-700 rounded-xl h-56 mb-6 overflow-hidden">
        <TripRouteMap
          points={tripPoints ?? []}
          places={(places ?? []).map((p) => ({ lat: p.lat, lng: p.lng, name: p.name, category: p.category }))}
        />
      </div>

      <section className="space-y-3">
        <h2 className="font-semibold text-white">Tempat yang Dikunjungi ({places?.length ?? 0})</h2>
        {places?.map((place) => (
          <div key={place.id} className="bg-slate-800 border border-slate-700 rounded-xl p-4">
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-slate-400 text-xs mt-0.5">{categoryLabel[place.category] ?? place.category}</p>
              </div>
              {place.reviews?.[0] && (
                <div className="text-yellow-400 text-sm">⭐ {place.reviews[0].rating}</div>
              )}
            </div>
            {place.reviews?.[0]?.notes && (
              <p className="text-slate-400 text-xs mt-2 italic">"{place.reviews[0].notes}"</p>
            )}
          </div>
        ))}
